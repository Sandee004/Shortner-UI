...
def index():
    if request.method == "POST":
        ...
        if current_user():
            # Logged-in user: No limit, create the link
            new_link = Shortener(long_link=long_link, user=current_user())
            ...
            # Non-logged-in user: Check link limit
            user_links_count = Shortener.query.filter_by(user_id=None).count()
            if user_links_count >= 5:
                success_message = "Link limit reached for non-logged-in users. Login to create more."
        ...
        context = {"success_message": success_message}
        links = Shortener.query.filter_by(user=current_user()).all()  # Filter links for current user
        return render_template("index.html", **context, links=links)

    # Render index page for logged-in or non-logged-in users
